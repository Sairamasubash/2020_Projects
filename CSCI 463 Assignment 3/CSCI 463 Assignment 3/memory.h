//***************************************************************************
//  File Name: memory.h
//
//  CSCI 463 - Assignment 5 - Fall 2020
//
//  Progammer: Sairamasubash Muppalaneni
//  Z-ID: Z1844762
//  Date Due: Monday, November 18, 2020
//***************************************************************************/

// Here are the beginning header guards for the memory.h file.
#ifndef memory_H
#define memory_H

// Here are all the libraries that are required by this header file.
#include <iostream>

class memory     // Start of the memory class.
{
    // Declaring the twelve public function prototypes for the memory class.
public:
    memory(uint32_t siz);
    ~memory();
    
    bool check_address(uint32_t i) const;
    uint32_t get_size() const;
    
    uint8_t get8(uint32_t addr) const;
    uint16_t get16(uint32_t addr) const;
    uint32_t get32(uint32_t addr) const;
    
    void set8(uint32_t addr, uint8_t val);
    void set16(uint32_t addr, uint16_t val);
    void set32(uint32_t addr, uint32_t val);
    
    void dump() const;
    
    bool load_file(const std::string &fname);
    
    // Declaring the two private data members for the memory class.
private:
    uint8_t *mem;
    uint32_t size;
};     // End of the memory class.

#endif     // Here is the ending header guard for the memory.h file.
